...
set(LLVM_INSTALL_PATH "" CACHE PATH "")

macro(llvmir_setup)
  set(LLVM_INSTALL_PATH_BIN "${LLVM_INSTALL_PATH}/bin/")

  set(LLVMIR_DIR "llvm-ir")

  set(LLVMIR_COMPILER "")
  set(LLVMIR_OPT "${LLVM_INSTALL_PATH_BIN}opt")
...
function(llvmir_extract_compile_defs_properties out_compile_defs from)
  ...
  get_property(defs DIRECTORY PROPERTY ${prop_name}_${UPPER_CMAKE_BUILD_TYPE})
  foreach(def ${defs})
    list(APPEND compile_defs -D${def})
  endforeach()
  ...
  if(TARGET ${from})
    ...
    get_property(defs TARGET ${from} PROPERTY ${prop_name}_${UPPER_CMAKE_BUILD_TYPE})
    foreach(def ${defs})
      list(APPEND compile_defs -D${def})
    endforeach()
    ...
    get_property(defs SOURCE ${from} PROPERTY ${prop_name}_${UPPER_CMAKE_BUILD_TYPE})
    foreach(def ${defs})
      list(APPEND compile_defs -D${def})
    endforeach()
  endif()
...
function(llvmir_extract_lang_flags out_lang_flags lang)
  ...
  set(lang_flags ${CMAKE_${lang}_FLAGS_${UPPER_CMAKE_BUILD_TYPE}})
...
function(llvm_check_valid_name_file name)
  get_filename_component(file ${in_name} NAME_WLE)
  get_filename_component(ext ${in_name} LAST_EXT)

  if(NOT ${PATH} STREQUAL ${file}${ext})
    message(FATAL_ERROR "Invalid file name ${in_name}.")
  endif()
endfunction()
